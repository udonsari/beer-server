...
* `중요` 토큰 Refresh, 만료 Client, Server 누가 처리하는지 알아보고 처리하기 ( -> Client가 알아서 만료 처리)
* `중요` 로그인 Token 자체를 Client에서 받게 하기. 서버는 Token 그냥 받고 (웹프론트에서는 필요할 것 같으니 남겨두자)
...
* `WIP` 맥주 사이즈 정하고 (360 * 260), 그에 맞게 데이터 가공 하기 (이거 미정님 가이드 받고 배워두기. 이미지 사이즈에 맞게 재가공 하는 것, 누끼 따는 것 ... 그 외 미정님한테 도움될만한거 묻기 ) + 데이터 몇개 까지 (100개) ?
* `WIP` 미정님한테 맥주 이미지 수정 부탁 -> 완료시 반영하여 서버 업데이트
...
* Client로부터 App version 받아서 로그 찍기 (File)
* 지금 kakao에서 user not found이면 User()에서 그 에러인 경우는 문제 없다 처리 필요. 현재 User() == UserMust() 처럼 동작